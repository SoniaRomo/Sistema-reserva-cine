...
def horarios():
    if 'loggedin' in session:
        cur = mysql.connection.cursor()
        cur.execute('SELECT h.id, p.titulo, s.nombre, h.fecha, h.hora FROM Horario h JOIN Pelicula p ON h.id_pelicula = p.id JOIN Sala s ON h.id_sala = s.id')
        horarios = cur.fetchall()
        return render_template('horarios.html', horarios=horarios)
    return redirect(url_for('login'))

@app.route('/add_horario', methods=['GET', 'POST'])
def add_horario():
    if 'loggedin' in session:
        cur = mysql.connection.cursor()
        if request.method == 'POST':
            ...
            cur.execute("SELECT id FROM Pelicula WHERE titulo = %s", [titulo_pelicula])
            pelicula = cur.fetchone()
            if not pelicula:
                flash("Película no encontrada.")
                return redirect(url_for('add_horario'))

            cur.execute("SELECT id FROM Sala WHERE nombre = %s", [nombre_sala])
            sala = cur.fetchone()
            if not sala:
            ...
            cur.execute(
                "INSERT INTO Horario (id_pelicula, id_sala, fecha, hora) VALUES (%s, %s, %s, %s)", 
                (pelicula[0], sala[0], fecha, hora)
            )
            mysql.connection.commit()
        ...
        cur.execute("SELECT titulo FROM Pelicula")
        peliculas = cur.fetchall()
        cur.execute("SELECT nombre FROM Sala")
        salas = cur.fetchall()


        return render_template('add_horario.html', peliculas=peliculas, salas=salas)
    return redirect(url_for('login'))

@app.route('/edit_horario/<int:id>', methods=['GET', 'POST'])
def edit_horario(id):
    if 'loggedin' in session:
        cur = mysql.connection.cursor()
        if request.method == 'POST':
            ...
            cur.execute(
                "UPDATE Horario SET id_pelicula=%s, id_sala=%s, fecha=%s, hora=%s WHERE id=%s", 
                (pelicula[0], sala[0], fecha, hora, id)
            )
            mysql.connection.commit()
            return redirect(url_for('horarios'))

        cur.execute("SELECT * FROM Horario WHERE id=%s", [id])
        ...
        return render_template('edit_horario.html', horario=horario, peliculas=peliculas, salas=salas)
    return redirect(url_for('login'))

@app.route('/delete_horario/<int:id>', methods=['GET', 'POST'])
